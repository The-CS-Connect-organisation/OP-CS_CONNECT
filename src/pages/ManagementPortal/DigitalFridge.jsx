...
import { Clock, Plus, CheckCircle, Trash2, X, AlertTriangle, Calendar, RefreshCw } from 'lucide-react';
...
export function DigitalFridge({ user, addToast }) {
  ...
  useEffect(() => {
    ...
    const handleFridgeUpdate = (data) => {
      if (data.type === 'fridge-task') {
        addToast?.(`📋 ${data.title || 'New task'} shared with you`, 'info');
        loadItems();
      }
    };
    socket.on('notification:new', handleFridgeUpdate);
    return () => { socket.off('notification:new', handleFridgeUpdate); };
  }, [loadItems, addToast]);

  const handleCreate = async () => {
    if (!formData.title) { addToast?.('Title required', 'error'); return; }
    try {
      await studentApi.createFridgeItem({ ...formData, sharedWith: formData.sharedWith.filter(Boolean) });
