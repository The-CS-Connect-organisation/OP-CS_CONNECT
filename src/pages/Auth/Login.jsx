...
export const Login = ({ onLogin, onSwitch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = onLogin(cleanEmail, cleanPassword);
    if (!result.success) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-950 dark:to-primary-950 p-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div animate={{ y: [0, -30, 0] }} transition={{ duration: 8, repeat: Infinity }} className="absolute -top-20 -right-20 w-96 h-96 bg-primary-200/30 dark:bg-primary-800/20 rounded-full blur-3xl" />
        <motion.div animate={{ y: [0, 30, 0] }} transition={{ duration: 10, repeat: Infinity }} className="absolute -bottom-20 -left-20 w-96 h-96 bg-purple-200/30 dark:bg-purple-800/20 rounded-full blur-3xl" />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
        <div className="glass-card p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {error && (
              <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-red-500 flex items-center gap-2">
                ⚠️ {error}
              </motion.p>
            )}
