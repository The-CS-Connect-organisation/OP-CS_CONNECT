  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <p className="text-xs text-[var(--text-muted)]">Drag and drop or click to upload</p>
        <p className="text-[11px] text-[var(--text-muted)] mt-1">jpg/png/webp up to 5MB, pdf up to 10MB</p>
      </div>
      {uploading && (
      ...
      {previewUrl && (
        <div className="relative rounded-xl border border-orange-500/20 bg-[#101010] p-2">
          {previewUrl.startsWith('data:image') ? (
            ...
            <p className="text-xs text-[var(--text-muted)] truncate">{previewUrl}</p>
          )}
          <button
            type="button"
            onClick={(e) => {
            ...
            className="absolute top-2 right-2 p-1 rounded-full bg-black/60 text-[var(--text-primary)]"
          >
            <X size={12} />
          </button>
        </div>
