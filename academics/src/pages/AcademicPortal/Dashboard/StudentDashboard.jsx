...
import { Clock, Award, FileText, UserCheck } from 'lucide-react';
import { useAssignments, useAttendance, useMarks, useAnnouncements } from '../../../hooks/useSchoolData';
...
export const StudentDashboard = ({ user }) => {
  ...
  const { data: timetableData } = useApi(
    classroomId ? `/school/timetables?classId=${classroomId}` : null,
    { defaultValue: null, skip: !classroomId }
  );

  const today = new Date().toLocaleDateString('en-US', { weekday: 'long' });
  const timetableEntries = timetableData?.entries || [];
  const todaySchedule = timetableEntries.find(d => d.day === today)?.slots || [];

  const attendanceRate = (user?.attendancePercent != null && user.attendancePercent > 0)
    ? user.attendancePercent
    : attendanceRecords.length > 0
      ? Math.round(attendanceRecords.filter(r => r.status === 'present').length / attendanceRecords.length * 100)
      : 0;

  const avgMarks = marksReport?.percentage != null ? Math.round(marksReport.percentage) : 0;

  const pendingAssignments = assignments.filter(a => {
    const due = new Date(a.due_date || a.dueDate);
    return due >= new Date(Date.now() - 7 * 86400000);
  });
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-semibold bg-blue-50 text-blue-600 border border-blue-200">
              ...
              {user.class || `${user.grade}-${user.section}`}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {[`Class: ${user.class || 'N/A'}`, `ID: ${user.rollNo || 'N/A'}`, today].map(tag => (
              <span key={tag} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-50 text-gray-600 border border-gray-200">{tag}</span>
            ))}
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {todaySchedule.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {todaySchedule.map((slot, idx) => (
                <div key={idx} className="flex items-center gap-3 p-3.5 rounded-xl bg-gray-50 border border-gray-200">
                  ...
                  <div key={a.id} className="p-3.5 rounded-xl bg-gray-50 border border-gray-200">
                    <div className="flex justify-between items-start gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs mt-1 text-gray-500">{a.subject} • Due: {dueDate?.split('T')[0]}</p>
                      </div>
                      {isLate && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-red-50 text-red-600 shrink-0">Overdue</span>}
                    </div>
                  </div>
                );
              })}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="nova-card p-6">
          ...
          <div className="space-y-3">
            {announcements.length === 0 && <p className="text-xs text-gray-500">No announcements</p>}
              ...
              <div key={a.id} className="border-l-2 pl-3 py-1.5 border-gray-200">
                <div className="flex justify-between items-center mb-0.5">
                  <span className={`text-[10px] font-semibold rounded-full px-2 py-0.5 capitalize ${
                    a.category === 'emergency' ? 'bg-red-50 text-red-600' :
                    a.category === 'exam' ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-gray-500'
                  }`}>{a.category}</span>
                  <span className="text-[10px] font-mono text-gray-400">{a.created_at?.split('T')[0]}</span>
                </div>
                <p className="text-sm font-semibold leading-tight mt-1 text-gray-800">{a.title}</p>
                <p className="text-xs mt-0.5 text-gray-500 line-clamp-2">{a.body}</p>
              </div>
            ))}
          </div>
        </motion.div>
