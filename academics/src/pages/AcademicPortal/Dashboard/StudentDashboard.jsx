...
export const StudentDashboard = ({ user }) => {
  ...
  // Attendance: use attendancePercent from user object (set at login), fallback to computed
  ...
  // Marks: percentage from report card
  const avgMarks = marksReport?.percentage != null ? Math.round(marksReport.percentage) : 0;

  // Pending assignments: those without a submission from this student
  // The API doesn't embed submissions in list, so show all as pending for now
  const pendingAssignments = assignments.filter(a => {
    const due = new Date(a.due_date || a.dueDate);
    return due >= new Date(Date.now() - 7 * 86400000); // due within last week or future
  });
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {[
              `Class: ${user.class || `${user.grade}-${user.section}` || 'N/A'}`,
              `ID: ${user.rollNo || 'N/A'}`,
              today
            ].map((tag) => (
              ...
              <div key={a.id} className="border-l-2 pl-3 py-1.5 border-gray-200">
                <div className="flex justify-between items-center mb-0.5">
                  <span className={`text-[10px] font-semibold rounded-full px-2 py-0.5 capitalize ${
                    ...
                    a.category === 'exam' ? 'bg-amber-50 text-amber-600' :
